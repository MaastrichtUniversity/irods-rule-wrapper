...
def test_rule_get_active_drop_zones():
    result = RuleManager("rodsadmin").get_active_drop_zones("false")
    assert result is not None


def test_generate_token():
    result = RuleManager("rodsadmin").generate_token()
    assert result is not None and result.token is not None


def test_rule_get_active_drop_zone():
    result = RuleManager("rodsadmin").get_active_drop_zone("adorable-barracuda", "true")
    assert result is not None
