...
def test_rule_create_ingest():
    result = RuleManager("rodsadmin").create_ingest("jmelius", "token1", "P000000014", "Title1")
...
def test_rule_create_drop_zone():
    ...
    token = RuleManager("rodsadmin").create_drop_zone(data)
    print(token)
...
def test_read_metadata_xml_dropzone():
    ...
    xml = RuleManager("rodsadmin").read_metadata_xml_from_dropzone(token)
    assert xml is not None


@pytest.mark.skip(reason="excluded from overall testing because these need specific setup.")
def test_rule_start_ingest():
    result = RuleManager("rodsadmin").start_ingest("jmelius", "token")
    assert result is None
